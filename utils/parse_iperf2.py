...
def topology(folder):
        ...
        for dst in range(num_node):
            for pri in ['h', 'l']:
                if src != dst and (src, dst) not in neglect:
                    ...
                    loss_list.append((loss1*100, loss2*100, total_send / 1000, pri, src, dst, total_send, total_receive))
    ...
    for loss1, loss2, traffic, pri, src, dst, ts, tr in loss_list:
        #print src, dst, loss1, loss2, traffic, ts, tr
        print loss2, traffic, src, dst, pri
