...
python create_scenario.py --scenario_file ../_data/scenario_ibm.tab --scenario_template ../_config/scenario_template.yaml
printf "**********************Copying _config _data to node 0.\n\n"
scp -r ../_config/* ${username}@${host_0}:~/testbed_mininet/_config/
# scp -r ../_config/* li3566@ms0322.utah.cloudlab.us:~/testbed_mininet/_config/
scp -r ../_data/* ${username}@${host_0}:~/testbed_mininet/_data/
scp -r server.txt ${username}@${host_0}:~/testbed_mininet/utils/
