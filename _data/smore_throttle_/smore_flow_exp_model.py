...
def _compute_teavar(main_config, topo_config, solver_config):
  logger = logging.getLogger('compute_ffc_tunnel')

  smore_flow_exp_solver = teavar.SmoreFlowExpModelSolver(
      main_config=main_config,
      topo_config=topo_config,
      solver_config=solver_config)

  res, solving_time = smore_flow_exp_solver.compute_max_throughput(0.841)
  logger.info("smore flow result = %s" % (res))
...
def _main(args, configs):
  ...
  logger.info("Start.")
  _compute_teavar(main_config, topo_config, solver_config)
  logger.info("Done.")
