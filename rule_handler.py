...
class Rule_handler(object):
    ...
    def configure_initial_split_1class(self, initial_file, rule_name):
        rules_set = defaultdict(list)
        logging.info("\tConfiguring path weights for single class...")
        groups_info = {}
    ...
    def configure_initial_split_2class(self, initial_file, rule_name):
        rules_set = defaultdict(list)
        logging.info("\tConfiguring path weights for two classes...")
        groups_info = {}
        with open(initial_file, 'r') as f:
            for line in f:
                if "s" in line and "shutdown" not in line:
                    continue
                if "throttle" in line:
                    _, s, t, prio, loss = line.strip().split(' ')
                    src_host_name = 'h{}_{}'.format(prio, s)
                    dst_host_name = 'h{}_{}'.format(prio, t)
                    self.topo.throttle_pair[src_host_name, dst_host_name] = float(loss)
                    continue
                if "shutdown" in line:
                    _, s, t, prio = line.strip().split(' ')           
                    src_switch_name = 's_{}'.format(s)
                    src_switch = self.topo.switch_set[src_switch_name]
                    dst_host_name = 'h{}_{}'.format(prio, t)
                    dst_host = self.topo.host_set[dst_host_name]
